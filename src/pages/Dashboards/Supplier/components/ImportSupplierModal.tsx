...
const ImportSupplierModal: React.FC<props> = ({
  show,
  file,
  onCancel,
  onDone,
  ...
  const columns = React.useMemo<ColumnDef<ExcelData>[]>(() => {
    ...
    return Object.keys(excelData[0]).map((key) => ({
      ...
      enableSorting: false,
      cell: (cell: any) => {
        const columnsMoneyTypes = ["Tổng mua", "Tổng mua trừ trả hàng", "Công nợ hiện tại"];
  ...
  const handleImport = async () => {
    ...
    try {
      const response: IHttpResponse = await request.post(
        `/suppliers/import?type=${importType}`,
        formData,
        {
